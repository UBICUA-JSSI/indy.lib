...
//        int indy_set_logger_with_max_lvl(Pointer context, Callback enabled, Callback log, Callback flush, int max_lvl);
     ...
     *
     * @param searchPath The path to the directory containing the C-Callable
     * library file.
     */
    ...
    /**
     * Initializes the API with the path to the C-Callable library. Warning:
...
//        api.indy_set_logger_with_max_lvl(null, Logger.enabled, Logger.log, Logger.flush, logLevel);
    ...
    /**
     * Set libindy runtime configuration. Can be optionally called to change
     * current params.
     *
     * @param config config: { "crypto_thread_pool_size": Optional[int] - size
     * of thread pool for the most expensive crypto operations. (4 by default)
     * "collect_backtrace": Optional[bool] - whether errors backtrace should be
     * collected. Capturing of backtrace can affect library performance. NOTE:
     * must be set before invocation of any other API functions. }
     */
